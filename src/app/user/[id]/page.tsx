...
export default function UserPage() {
  ...
  const [editUser, setEditUser] = useState<User | null>(null); // 🔹 state untuk user yang sedang diedit
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setUsers((prev) =>
          prev.map((u) => (u._id === editUser._id ? data.data : u))
        );
        setEditUser(null);
      } else {
        alert(data.message || "Gagal mengubah data user.");
      }
    } catch (err) {
      console.error(err);
